...
```
## [1] "Sample.id"             "cell.type"             "cell.line.tissue.type"
## [4] "gender"                "Type"
```
...
![plot of chunk data cleaning and normalization](figure/data_cleaning_and_normalization1.png) 
...
![plot of chunk data cleaning and normalization](figure/data_cleaning_and_normalization2.png) 
...
heatmap(as.matrix(log.avgExpCleanNorm[c(row.names(topTableType[1:100, ])), ]), 
    scale = "none", Rowv = NA, main = "clustering top 100 genes")
```

![plot of chunk limma](figure/limma.png) 
...
genesOfInterest <- factor(c("LIN28", "ZSCAN10", "POU5F1P1", "HDAC4", "LOC643911"), 
    ordered = F)

jDat <- prepareData(log.avgExpCleanNorm, genesOfInterest, shortMeta)

jDat$gene <- factor(jDat$gene, ordered = T, levels = c("LIN28", "ZSCAN10", "POU5F1P1", 
    "HDAC4", "LOC643911"))
...
ggplotLayer1 <- ggplotLayer1 + stat_summary(fun.y = mean, aes(group = 1), geom = "line", 
    color = "black")
ggplotLayer1 <- ggplotLayer1 + facet_grid(~gene)
((ggplotLayer1 <- ggplotLayer1 + theme_bw() + xlab("Genotype") + ylab("Gene Expression log2") + 
    ggtitle("Gene Expression for 3  top hits, and 2 non-hits")))
```

![plot of chunk making FDR cut offs and top genes](figure/making_FDR_cut_offs_and_top_genes.png) 
